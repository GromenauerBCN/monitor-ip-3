...
function botApi(path) {
    const token = process.env.TELEGRAM_BOT_TOKEN;
    if (!token)
        throw new Error('TELEGRAM_BOT_TOKEN no definit');
...
export function telegramRouter(app) {
    app.post('/api/telegram/webhook', async (req, reply) => {
        ...
        if (text.startsWith('/start')) {
            ...
            if (!token) {
                await sendTelegram(chatId, 'Benvingut/da! Per vincular el teu compte, ves a Configuració > Telegram i envia el codi que hi veus: /start <codi>.');
                return { ok: true };
            }
...
export async function notifyTelegram(userId, text) {
    try {
