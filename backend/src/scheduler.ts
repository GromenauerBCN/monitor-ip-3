...
function minutesSince(d?: Date | null) {
 ...
 */
async function runOnceAndGetOk(targetId: string): Promise<{ ok: boolean; error?: string }> {
  ...
  async () => {
    ...
    const dueCandidates = await prisma.monitorTarget.findMany({
      where: {
        ...
        OR: [
          { lastCheckedAt: null },
          { lastCheckedAt: { lte: new Date(now.getTime() - 60_000) } },
        ],
      },
    ...
    for (const t of due) {
      const prevStatus = (t as any).lastStatus || 'UNKNOWN';
      ...
      try {
        ...
        for (let attempt = 1; attempt <= RETRY_ATTEMPTS; attempt++) {
          if (DEBUG_RETRIES) {
            console.log(
              `[scheduler][${new Date().toLocaleTimeString()}] ${t.label} intent ${attempt}/${RETRY_ATTEMPTS}`
            );
          }
          ...
          if (attempt < RETRY_ATTEMPTS) {
            console.log(
              `[scheduler] "${t.label}" KO (intent ${attempt}/${RETRY_ATTEMPTS}). Reintento en ${RETRY_INTERVAL_MS / 1000}s…`
            );
        ...
        if (!finalOk && fresh) {
          ...
          if (DEBUG_RETRIES) {
            ...
            console.log(
              `[scheduler] Decide alert for "${fresh.label}": finalOk=${finalOk}, prevStatus=${prevStatus}, ` +
          ...
          if (shouldAlert) {
            ...
            try { await sendPush(fresh.userId, 'Servei caigut', `${fresh.label} no respon`); } catch {}
            // telegram (si tens bot connectat)
            try { await notifyTelegram(fresh.userId, `⚠️ Servei caigut: ${fresh.label}`); } catch {}
            ...
            if (DEBUG_RETRIES) console.log(`[scheduler] ALERT path finished for "${fresh.label}"`);
          } else if (DEBUG_RETRIES) {
            console.log(
              `[scheduler] Alert suppressed for "${fresh.label}" (prevStatus=${prevStatus}, cooldownOk=${cooldownOk})`
            );
          }
        }
        ...
        if (finalOk && prevStatus === 'DOWN') {
          ...
          if (notif?.emailRecipients?.length) {
            try { await sendEmail(notif.emailRecipients, subject, html); } catch {}
          }
          try { await sendPush(t.userId, 'Servei recuperat', `${t.label} torna a respondre`); } catch {}
          try { await notifyTelegram(t.userId, `✅ Servei recuperat: ${t.label}`); } catch {}
          await prisma.monitorTarget.update({
            where: { id: t.id },
            data: { lastStatus: 'UP' },
          });
...
cron.schedule(
  ...
  async () => {
    ...
    for (const u of users) {
      ...
      if (!notif?.dailyDigest || !notif.emailRecipients?.length) continue;
...
cron.schedule(
  ...
  async () => {
    ...
    for (const u of users) {
      const notif = await prisma.notificationSettings.findUnique({ where: { userId: u.id } });
      if (!notif?.monthlyDigest || !notif.emailRecipients?.length) continue;
      const errors = await prisma.checkResult.findMany({
